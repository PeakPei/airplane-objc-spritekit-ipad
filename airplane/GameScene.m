...
-(void)EnemiesAndClouds{
    ...
    if (randomClouds == 1) {
        int whichClouds = [self getRandomNumberBetween:0 to:3];
        SKSpriteNode *cloud = [SKSpriteNode spriteNodeWithTexture:[_cloudsTextures objectAtIndex:whichClouds]];
        int randomYAxis = [self getRandomNumberBetween:0 to:screenRect.size.height];
        cloud.position = CGPointMake(screenRect.size.height + cloud.size.height/2, randomYAxis);
        cloud.zPosition = 1;
        int randomTimeCloud = [self getRandomNumberBetween:9 to:19];
        
        SKAction *move = [SKAction moveTo:CGPointMake(0 - cloud.size.height, randomYAxis) duration:randomTimeCloud];
        SKAction *remove = [SKAction removeFromParent];
